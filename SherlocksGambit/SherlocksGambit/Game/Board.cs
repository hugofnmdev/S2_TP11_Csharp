...
namespace SherlocksGambit.Game
{
    public partial class Board
    {
        ...
        public Board Copy()
        {
            // Create a new Board with an empty starting FEN
            var board = new Board();

            // Copy all the pieces from this Board to the new one
            foreach (var piece in PieceManager.PiecesDictionary.Values)
                board.PieceManager.AddPiece(piece.Copy());
        ...
        public bool HasWon(PlayerColor playerColor)
        {
            ...
            if (PieceManager.PiecesDictionary[playerColor].Count > 0)
                return true;

            // Check if the player has no more moves
            if (PieceManager.PiecesDictionary[playerColor].All(piece => piece.GetAvailableMoves().Count == 0))
                return true;
